...
root1 = 'C:/Users/hb/Desktop/data/CIFAR10'
root2 = 'C:/Users/hb/Desktop/data/cifar10Z'
...
with open(root2+'/batches.meta', 'rb') as infile:
    data = pickle.load(infile, encoding='latin1')
    classes = data['label_names']

# 클래스 별 폴더 생성
os.mkdir(root1 + '/train')
os.mkdir(root1 + '/test')
for name in classes:
    os.mkdir(root1 + '/train/{}'.format(name))
    os.mkdir(root1 + '/test/{}'.format(name))
# Trainset Unpacking
# data_batch 파일들 순서대로 unpacking
for i in range(1, 6):
    print('Unpacking Train File {}/{}'.format(i, 5))
    train_file = unpickle( root2 + '/data_batch_{}'.format(i))
    ...
    for idx in range(10000):
        ...
        # 클래스 별 폴더에 파일 저장
        train_image.save(root1 + '/train/{}/{}'.format(classes[train_label], train_filename[idx].decode('utf8')))
# -----------------------------------------------------------------------------------------
# Testset Unpacking
print('Unpacking Test File')
test_file = unpickle(root2 + '/test_batch')
...
for idx in range(10000):
    ...
    # 클래스 별 폴더에 파일 저장
    test_image.save(root1 + '/test/{}/{}'.format(classes[test_label], test_filename[idx].decode('utf8')))
